...
public class UpdateRoleUsersCommandHandler : IRequestHandler<UpdateRoleUsersCommand, IResult>
{
    ...
    )
    {
        ...
        string addedUsersError = string.Empty;
        if(!isAddedUsersIdsEmpty)
        {
            // if any user not found
            var foundUsers = await _rawDBService.CountAsync<User>(e => command.AddedUsersIds.Contains(e.Id));
            if(foundUsers < command.AddedUsersIds.Count)
                addedUsersError = "one or more User(s) not found!!!";
            else
            {
                ...
                if(existingRoleUsers.Any(e => command.AddedUsersIds.Contains(e.UserId)))
                    addedUsersError = "one or more User(s) already in This Role!!!";
                else
                {
                    // do assign Role To Users
        ...
        // return the result
        var addedUsersSucceeded = string.IsNullOrEmpty(addedUsersError);
        ...
        if(addedUsersSucceeded && removedUsersSucceeded)
            ...
            return _resultService.Conflict(nameof(UpdateRoleUsersCommand), string.Join(", ", addedUsersError, removedUsersError));
            ...
            return _resultService.Conflict(nameof(UpdateRoleUsersCommand), $"Role [{command.RoleId}] => unassigned from users successfully, but there is errors in assigned users: {addedUsersError}");
    }
}
